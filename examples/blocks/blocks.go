...
func setupScenario(sim *rvo.RVOSimulator) {
	...
	sim.SetAgentDefaults(15.0, 10, 5.0, 5.0, 1.5, 0.2, &rvo.Vector2{}) // where is velocity property ?

	for i := 0; i < 5; i++ {
		for j := 0; j < 5; j++ {
			id1, _ := sim.AddDefaultAgent(&rvo.Vector2{X: 55.0 + float32(i)*10.0, Y: 55.0 + float32(j)*10.0})
			sim.SetAgentGoal(id1, &rvo.Vector2{X: -75.0, Y: -75.0})

			id2, _ := sim.AddDefaultAgent(&rvo.Vector2{X: -55.0 - float32(i)*10.0, Y: 55.0 + float32(j)*10.0})
			sim.SetAgentGoal(id2, &rvo.Vector2{X: 75.0, Y: -75.0})

			id3, _ := sim.AddDefaultAgent(&rvo.Vector2{X: 55.0 + float32(i)*10.0, Y: -55.0 - float32(j)*10.0})
			sim.SetAgentGoal(id3, &rvo.Vector2{X: -75.0, Y: 75.0})

			id4, _ := sim.AddDefaultAgent(&rvo.Vector2{X: -55.0 - float32(i)*10.0, Y: -55.0 - float32(j)*10.0})
